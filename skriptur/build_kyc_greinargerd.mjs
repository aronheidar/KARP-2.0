...
//  kyc_adverse, kyc_tonn, kyc_event) + EIN LLM-samantektar-málsgrein sem talna-gátin
//  (parseTulkun) ver gegn hallucination — tala sem ekki er í samhenginu fellir túlkunina
//  og kaflar 1–4 standa þá sjálfstætt. Sjá lib/kyc-greinargerd.mjs.
//
...
//      node skriptur/build_kyc_greinargerd.mjs [--dry]
// =============================================================================
import { erLogadili, greinargerdSamhengi, greinargerdHash, parseTulkun, GREINARGERD_SYSTEM } from '../web/src/lib/kyc-greinargerd.mjs';
...
async function tulka(samhengi) {
  ...
  for (let a = 0; a < 3; a++) {
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: MODEL, max_tokens: 600, system: GREINARGERD_SYSTEM, messages: [{ role: 'user', content: user }] }),
      });
  ...
  return null;   // kaflar 1–4 standa sjálfstætt — greinargerðin er samt gild
}

(async () => {
  const t0 = Date.now();
  const watches = await q("SELECT DISTINCT kt, nafn FROM kyc_watch WHERE status='active'");
  let ny = 0, obreytt = 0, einstakl = 0;
  for (const w of watches) {
    ...
    const samhengi = greinargerdSamhengi({ kt, nafn: w.nafn }, states, adverse, tonn, events);
  ...
  console.log('✔ greinargerðir: ' + ny + ' endurmyndaðar · ' + obreytt + ' óbreyttar (núll tóken) · ' + einstakl + ' einstaklingar sleppt · ' + ((Date.now() - t0) / 1000).toFixed(0) + 's');
