...
// Halli→skuldir vaxandi: nota RÍKJANDI-halla sviðsmynd (útgjaldaaukning). Skattalækkun er nú að hluta sjálf-fjármagnandi
// gegnum vöxt (gdp_debt nefnara-liður) — rétt módel-hegðun en ekki hrein halla-uppsöfnun. Útgjöld+12 → stór halli.
const expUp = simulate({ baseline, links, levers: { utgjold: 12 }, quarters: 12 });
const okDebtAccum = (expUp.outcomes.skuldir.mid[11] - expUp.outcomes.skuldir.baseline[11]) > (expUp.outcomes.skuldir.mid[3] - expUp.outcomes.skuldir.baseline[3]); // aukaskuldir v/hallans vaxa (frávik frá BAU)
...
// LAFFER (Ísland háskatta-land): skattahækkun hefur MINNKANDI jaðar-áhrif á afkomu → tvöföldun hækkunar skilar ekki tvöföldum tekjum
const laf6 = simulate({ baseline, links, levers: { skattar: 6 }, shocks: {}, quarters: 12 }).outcomes.afkoma.mid[11] - bAud.outcomes.afkoma.mid[11];
const laf12 = simulate({ baseline, links, levers: { skattar: 12 }, shocks: {}, quarters: 12 }).outcomes.afkoma.mid[11] - bAud.outcomes.afkoma.mid[11];
const okLaffer = laf6 > 0 && laf12 < 1.5 * laf6; // +12% skilar UNDIR 1,5× af +6% (mettun) — hindrar að KARP mæli með stórum skattahækkunum
console.log('Laffer — skattahækkun minnkandi jaðar-tekjur (+12% < 1,5×+6%):', okLaffer, `(+6%=${laf6.toFixed(2)}, +12%=${laf12.toFixed(2)})`);
// Nýjar íslenskar ákvarðanir: átt
const okVt = simulate({ baseline, links, levers: { verdtrygging: 80 }, shocks: {}, quarters: 12 }).outcomes.greidslubyrdi.mid[11] < bAud.outcomes.greidslubyrdi.mid[11] - 1e-9;
const okCapg = simulate({ baseline, links, levers: { fjarmagnstekjuskattur: 15 }, shocks: {}, quarters: 12 }).outcomes.jofnudur.mid[11] > bAud.outcomes.jofnudur.mid[11] + 1e-9;
const okPayroll = simulate({ baseline, links, levers: { tryggingagjald: 8 }, shocks: {}, quarters: 12 }).outcomes.atvinnuleysi.mid[11] > bAud.outcomes.atvinnuleysi.mid[11] + 1e-9;
console.log('nýjar ákvarðanir — verðtrygging→greiðslubyrði↓:', okVt, '| fjármagnstekjusk→jöfnuður↑:', okCapg, '| tryggingagjald→atvinnul.↑:', okPayroll);
// Tvíhliða-jöfnuður: þjóðhagsvarúð hefur nú FÓRNARSKIPTI (herðing bætir húsnæði EN lækkar hagvöxt) — ekki lengur „ókeypis"
...
// Lota 2: nýjar rásir/sleðar (áttir)
const okWorldSpread = simulate({ baseline, links, shocks: { heimshagvoxtur: -4 }, quarters: 12 }).outcomes.vaxtaalag.mid[4] > bAud.outcomes.vaxtaalag.mid[4] + 1e-9; // heimskreppa → álag↑ (risk-off)
const okByggdBal = simulate({ baseline, links, levers: { byggdastefna: 30 }, quarters: 12 }).outcomes.afkoma.mid[8] < bAud.outcomes.afkoma.mid[8] - 1e-9; // byggðastefna kostar nú
const okEldi = simulate({ baseline, links, levers: { fiskeldi: 40 }, quarters: 12 }).outcomes.utflutningur.mid[6] > bAud.outcomes.utflutningur.mid[6] + 1e-9; // fiskeldi → útflutningur↑
const okVotl = simulate({ baseline, links, levers: { votlendi: 40 }, quarters: 12 }).outcomes.losun.mid[10] < bAud.outcomes.losun.mid[10] - 1e-9; // votlendi → losun↓
const okWorldEq = simulate({ baseline, links, shocks: { heimshagvoxtur: 3 }, quarters: 12 }).outcomes.hlutabref.mid[4] > bAud.outcomes.hlutabref.mid[4] + 1e-9; // heimsuppsveifla → hlutabréf↑
console.log('lota2 — heimskreppa→álag↑:', okWorldSpread, '| byggðastefna→afkoma↓:', okByggdBal, '| fiskeldi→útflutn↑:', okEldi, '| votlendi→losun↓:', okVotl, '| heimsuppsveifla→hlutabréf↑:', okWorldEq);
const bad = !(okDir && okHouse && okGdp && okBand && okFrHouse && okMigHouse && okMigRent && okRateBurden && okHouseBand && okMigPop && okMigLabor && okMigGdp && okFerPop && okDemoBand && okTaxBal && okAdhBal && okDebtAccum && okFiscBand && okKvExp && okOrExp && okOrEmis && okCarbEmis && okResBand && okKaupGdp && okTourRent && okRateBal && okRateArrears && okTourArrears && okArrGdp && okArrBand && okLongFinite && okLongClamp && okLongBand && okLongLen && okPopStock && okMigPopLvl && okMigDep && okPenDep && okPenBal && okDemo7Band && okByggdUp && okOrkaByggd && okMigByggd && okByggdBand && okTaxKaup && okTaxInnov && okHvatiInnov && okInnovGdp && okKvFisk && okKvFiskUp && okCarbInfl && okMod9Band && okWorldExp && okVskInfl && okVskKaup && okDstiHouse && okBindHouse && okTransfKaup && okInnvGdp && okVeidiBal && okSkiptiEmis && okFridunFisk && okTourfeeBal && okMod10Band && okHbsRate && okHbsLandSym && okByggdLand && okFrLand && okFrHbs && okRegionConsist && okMenntGdp && okMod11Band && okRateFx && okFxDisinfl && okTourHbs && okMod12 && okRateCredit && okRateEquity && okExpCA && okCaNiip && okTransfEq && okCommExp && okMod13 && okSaturation && okAccel && okArrIdentity && okPhaseDiffers && okSFC && okFwdExp && okFwdBackcompat && okCarbonInnov && okRetireLabor && okEduUnem && okSpreadFx && okLaffer && okVt && okCapg && okPayroll && okDstiTO && okBindTO && okAcctCap && okUntested && okWorldSpread && okByggdBal && okEldi && okVotl && okWorldEq);
